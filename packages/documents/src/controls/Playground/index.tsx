...
import {get, capitalize, isEqual} from 'lodash-es';
...
class Playground extends React.PureComponent<Props, State> {
  ...
  public componentDidUpdate(prevProps: Props, prevState: State) {
    ...
    if (!isEqual(prevProps.defaultComProps, props.defaultComProps)) this.setState({ currentComProps: props.defaultComProps });
  }
  ...
  private reset = (nextComProps: ComProps) => {
    console.log(nextComProps, this.state.currentComProps);
        ...
        >
          ...
          {hideCode ? null : (
            <div className={PlaygroundLess.center}>
              ...
              <div
                className={`${PlaygroundLess.editorWrapper} ${!expand ? PlaygroundLess.constraint : ''}`}
              ...
              {expand ? null : <div className={PlaygroundLess.expand} onClick={this.toggleExpand}>
                <span>{">"}</span>
              </div>}
            </div>
          )}
