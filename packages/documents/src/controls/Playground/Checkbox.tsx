  ...
  return (
    <div className={wrapperClass.join(' ')}>
      {options.map((opt) => {
        ...
        const isDisabled = isBoolean(opt.isDisabled) ? opt.isDisabled : opt.isDisabled ? opt.isDisabled(currentProps) : false;
        return (
          <label
            key={opt.label}
            className={`${PlaygroundLess.label} ${isDisabled ? PlaygroundLess.labelDisabled : ''}`}
          >
            <input onChange={ () => onOptClick(opt)} type="checkbox" value="Bike" checked={isChecked} disabled={isDisabled} />
            <span>{opt.label}</span>
          </label>
        )
      })}
...
export const renderCheckbox = (options: Props['options'], direction: 'horizontal' | 'vertical') => {
  return (currentProps: ComProps, reset: ResetComProps) => {
    return <Checkbox reset={reset} currentProps={currentProps} options={options} direction={direction}/>;
  }
}
